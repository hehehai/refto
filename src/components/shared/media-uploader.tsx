...
export const MediaUploader = forwardRef<HTMLDivElement, MediaUploaderProps>(
  (
    ...
    const urlNeedSyncStorage = useMemo(
      () => (fileUrl ? needSyncStorage(fileUrl) : false),
    ...
    return (
      ...
      >
        <label htmlFor={`file_uploader_${name}`}>
          <div
            className={cn(
              "relative flex h-24 items-center justify-center overflow-hidden rounded-md border border-transparent bg-background focus-within:ring-2 focus-within:ring-zinc-700 focus-within:ring-offset-2 hover:border-zinc-700 dark:hover:border-zinc-200 dark:focus-within:ring-zinc-200",
              {
          ...
          >
            {previewInfo ? (
              <div className="group relative h-full w-full">
                ...
                <IconButton
                  className="invisible absolute top-1 right-1 bg-zinc-950/45 text-md backdrop-blur-xs group-hover:visible dark:bg-slate-200/45"
        ...
        <div className="mt-1 flex items-center justify-between space-x-2">
          {errorMessage ? (
            <div className="py-1 font-medium text-destructive text-xs">
              {errorMessage}
            </div>
          ) : (
